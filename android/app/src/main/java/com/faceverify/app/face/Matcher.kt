...
object Matcher {
    ...
    /** 1:1 — does the probe match this person's stored embeddings? */
    fun verify(probe: FloatArray, embeddings: List<FloatArray>): Decision {
        val s = bestScore(probe, embeddings)
        return Decision(s >= Config.MATCH_THRESHOLD, null, s, 0f)
    }
    ...
    fun identify(probe: FloatArray, people: List<Pair<String, List<FloatArray>>>): Decision {
        ...
        val granted = top >= Config.MATCH_THRESHOLD &&
            (scored.size == 1 || margin >= Config.IDENTIFY_MARGIN)
